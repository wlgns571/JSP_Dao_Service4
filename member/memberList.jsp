		...
		<nav class="text-center">
			<ul class="pagination">
				...
				<c:if test="${searchVO.firstPage ne 1 }">
					<li><a href="memberList.jsp?curPage=${searchVO.firstPage -1}" data-page="${searchVO.firstPage -1}"><span aria-hidden="true">&lt;</span></a></li>
				</c:if>
				...
				<c:forEach begin="${searchVO.firstPage }" end="${searchVO.lastPage }" var="i">
		        	...
		        	<c:if test="${searchVO.curPage ne i }"><li><a href="memberList.jsp?curPage=${i }"  data-page="${i }" >${i }</a></li></c:if>
				</c:forEach>
				<!-- 다음  페이지  -->
				<c:if test="${searchVO.lastPage ne searchVO.totalPageCount }">
					<li><a href="memberList.jsp?curPage=${searchVO.lastPage +1}" data-page="${searchVO.lastPage +1}"><span aria-hidden="true">&gt;</span></a></li>
				</c:if>
